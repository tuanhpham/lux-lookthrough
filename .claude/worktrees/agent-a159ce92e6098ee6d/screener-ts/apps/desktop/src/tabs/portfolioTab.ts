...
function toolbarHtml(): string {
  return `<div class="toolbar">
      ...
      ${activeId !== OVERVIEW_ID ? `<button id="acct-edit" class="range-btn">✎ Edit account</button>
      <button id="acct-delete" class="range-btn">🗑 Delete account</button>` : ''}
      <button id="acct-update" class="btn" style="margin-left:auto"${activeId === OVERVIEW_ID ? ' disabled title="Switch to an individual account to update prices"' : ''}>↻ Update prices</button>
    </div>`;
}
...
function transactionHistoryHtml(st: AccountState): string {
  ...
  const body = rows
    ...
    .join('');
  return `${avgLine}<table style="white-space:nowrap"><thead><tr>
